...
const outDir = path.join(__dirname, 'public/screenshots/ios')
...
const PW = 1000
const SW = PW - 60
const SH = Math.round(SW * 17 / 9)
const PH = SH + 140
const PX = (W - PW) / 2
const PY = 291
const SX = PX + 30, SY = PY + 60
...
for (const s of screens) {
  ...
  ctx.lineWidth = 5
  roundRect(ctx, PX + 2, PY + 2, PW - 4, PH - 4, 68)
  ctx.stroke()
  ...
  // iPhone buttons: Action button + Volume up/down on left, Side button on right
  ctx.fillStyle = '#2a2a2a'
  ctx.beginPath(); ctx.roundRect(PX - 6, PY + 200, 7, 45, 3); ctx.fill()  // Action button
  ctx.beginPath(); ctx.roundRect(PX - 6, PY + 290, 7, 75, 3); ctx.fill()  // Volume up
  ctx.beginPath(); ctx.roundRect(PX - 6, PY + 385, 7, 75, 3); ctx.fill()  // Volume down
  ctx.beginPath(); ctx.roundRect(PX + PW - 1, PY + 290, 7, 110, 3); ctx.fill() // Side/power
  ...
  // Dynamic Island — pill shape, iPhone 15/16 Pro style
  ctx.fillStyle = '#0a0a0a'
  roundRect(ctx, PX + PW / 2 - 65, SY + 14, 130, 38, 19)
  ctx.fill()

  // Home indicator — thin pill at bottom of screen
  ctx.fillStyle = 'rgba(255,255,255,0.45)'
  roundRect(ctx, PX + PW / 2 - 65, SY + SH - 22, 130, 6, 3)
  ctx.fill()

  // Caption
  const capY = PY + PH + 200
